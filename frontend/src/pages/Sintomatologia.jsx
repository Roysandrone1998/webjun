    ...
    const cargarContenido = async () => {
      try {
        ...
        const filtrados = data.filter(
          item => item.section === 'mente' && item.category === 'sintomatologia'
        );
        setArticulos(filtrados);
      } catch (error) {
        console.error("Error al cargar Sintomatología:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div style={styles.container}>
      ...
      <header style={styles.header}>
        <span style={styles.subtitulo}>EQUIPO QNH</span>
        <h1 style={styles.titulo}>Sintomatología</h1>
        <p style={styles.bajada}>Guías de reconocimiento, alertas y registros de síntomas sin alarmarse.</p>
      </header>
      ...
      <div style={styles.listaArticulos}>
        {loading ? (
          ...
          articulos.map((art, index) => (
            <div key={art._id} style={styles.filaArticulo}>
              <div style={styles.leftFila}>
                ...
                <span style={styles.tagFila}>SÍNTOMA</span>
          ...
          <p style={styles.aviso}>No hay guías de síntomas cargadas por el momento.</p>
        )}
      </div>
    </div>
  );
...
// Estilos compartidos (Gris Neutro #8A857C)
const styles = {
  container: { padding: '4rem 10%', backgroundColor: '#FAF8F5', minHeight: 'calc(100vh - 70px)', fontFamily: 'sans-serif' },
  volverLink: { textDecoration: 'none', color: '#8A857C', fontSize: '0.8rem', fontWeight: 'bold', letterSpacing: '1px', display: 'inline-block', marginBottom: '2rem' },
  header: { marginBottom: '4rem', maxWidth: '700px' },
  subtitulo: { fontSize: '0.75rem', letterSpacing: '2px', color: '#8A857C', fontWeight: 'bold', display: 'block', marginBottom: '0.5rem' },
  ...
  tagFila: { fontSize: '0.7rem', color: '#8A857C', border: '1px solid #8A857C', padding: '4px 14px', borderRadius: '15px', fontWeight: '600', letterSpacing: '1px' },
